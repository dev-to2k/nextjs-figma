  ...
  const getIcon = (techName: string) => {
    switch (techName) {
      case "Java Spring Boot":
        return <SiSpringboot className="text-foreground/40" />;
      case "Angular":
        return <SiAngular className="text-foreground/40" />;
      case "Kafka":
        return <SiApachekafka className="text-foreground/50" />;
      case "MSSQL Stored Procedures":
        return <DiMsqlServer className="text-foreground/40" />;
      case "Oracle":
        return <SiOracle className="text-foreground/40" />;
      case "JWT & Encryption":
        return <SiJsonwebtokens className="text-foreground/50" />;
      case "TypeScript":
        return <SiTypescript className="text-foreground/40" />;
      case "Next.js":
        return <SiNextdotjs className="text-foreground/60" />;
      case "Bootstrap":
        return <SiBootstrap className="text-foreground/40" />;
      case "Tailwind CSS":
        return <SiTailwindcss className="text-foreground/40" />;
      default:
        return null;
    }
  };
  return (
    <span className="tech-icon flex items-center gap-2 px-4 py-2.5 rounded-xl bg-secondary border border-border text-muted-foreground hover:text-foreground hover:border-foreground/20 hover:bg-foreground/5 transition-all duration-300 cursor-default group">
      {getIcon(name)}
          ...
          <div className="work-bottom flex flex-col items-center text-center">
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: t("workExperience.bottomText"),
              }}
            />
            <h2 className="work-title text-2xl md:text-3xl font-bold text-foreground mb-12 tracking-tight">
              {t("workExperience.title")}
            </h2>

            <div className="relative">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-24 bg-foreground/[0.02] blur-3xl rounded-full -z-10" />
              <div className="flex flex-wrap justify-center gap-3 md:gap-4 items-center max-w-3xl">
                {techStack.map((tech) => (
